	...
	<table cellpadding="0" cellspacing="0" summary="지난투표결과 내용입니다." border="1" >
		...
		<thead>
			...
			<tr>
				...
				<td><span style="font-weight:bold;"><%= mobileResultTitle.getPollTitle() %></span></td>
			</tr>
			<tr>
				<th scope="row">내용</th>
				<td><%= mobileResultTitle.getPollContents().replaceAll("<", "&lt;") %></td>
			</tr>
			<tr>
		...
		<tr>
			...
			%>
				...
				<h4><%= mobileResult.getQuestNo() %>. <%= mobileResult.getQuestTitle().replaceAll("<", "&lt;") %> (주관식 문제입니다.)</h4>
				<% }else{%>
				<h4><%= mobileResult.getQuestNo() %>. <%= mobileResult.getQuestTitle().replaceAll("<", "&lt;") %></h4>
				<% } %>
			<%
				}else{
			%> 
				...
				%>
						<ul>
							<li style="padding: 4px;"><%= ExamNo %>. <%= mobileResult.getQuestTitle() %> <br /><img src="/img/poll/<%= ImgUrl %>" width="<%= AvgeGraph %>" height="15" border="0" alt="<%= Avge %>%"/> <%= mobileResult.getCntHh101() %>명 <%= Avge %>%</li>
						</ul> 
				<%
				...
				%>
						<ul>
						<li style="padding: 4px;"><%= ExamNo %>. <%= mobileResult.getQuestTitle() %> <br /><img src="/img/poll/<%= ImgUrl %>" width="<%= Avge2Graph %>" height="15" border="0" alt="<%= Avge2 %>%"/> <%= mobileResult.getCnt() %>명 <%= Avge2 %>%</li>
						</ul>
				<%		
				...
				%>
						...
						<ul>
						<li style="padding: 4px;"><%= ExamNo %>. <%= mobileResult.getQuestTitle() %> <br /><img src="/img/poll/<%= ImgUrl %>" width="<%= Avge4Graph %>" height="15" border="0" alt="<%= Avge4 %>%"/> <%= mobileResult.getCntHh104() %>명 <%= Avge4 %>%</li>
						</ul>
				<%		
					}
				%>
