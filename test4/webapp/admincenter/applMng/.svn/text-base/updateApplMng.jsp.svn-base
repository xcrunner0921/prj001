					...
					<tr>
						...
						<td>
							<select name="pollAsk" id="" class="ip100" required fieldTitle="의뢰부서">
								...
								%>
								<option value="<%= askCode.getCodeId() %>" <%= pollMng.getPollAsk().equals(askCode.getCodeId()) ? "selected" : "" %>><%= askCode.getCodeName() %></option>
								<%
									}
								%>
							</select>
...
<!--							<a href=""><img src="../../img/adminCenter/icon_month.gif" alt="등록일 달력"/></a>-->
					...
					<tr>
						...
						<td>
							<input type="text" name="" id="" title="" class="ip100" value="<%= pollMng.getPollSample() %>" maxlength="5" num="i" required fieldTitle="참여자수" />
						</td>
					</tr>
					<tr>
						...
						<td>
							<select name="pollCompany" id="" required fieldTitle="조사기관">
								...
								%>
								<option value="<%= companyCode.getCodeId() %>" <%= pollMng.getPollCompany().equals(companyCode.getCodeId()) ? "selected" : "" %>><%= companyCode.getCodeName() %></option>
								<%
									}
								%>
							</select>
						...
						<td>
							<select name="pollMethod" id="" required fieldTitle="조사방법">
								...
								%>
								<option value="<%= methodCode.getCodeId() %>" <%= pollMng.getPollMethod().equals(methodCode.getCodeId()) ? "selected" : "" %>><%= methodCode.getCodeName() %></option>
								<%
									}
								%>
							</select>
