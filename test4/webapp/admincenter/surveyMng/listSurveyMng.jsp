...
<%
	...
	String regDatestart = EmpUtil.null2str(surveyMngSearchVO.getSearchRegDatestart());
	String regDateend = EmpUtil.null2str(surveyMngSearchVO.getSearchRegDateend());
	String pollStop = EmpUtil.null2str(surveyMngSearchVO.getSearchPollStop());
	...
	String keyword = EmpUtil.null2str(surveyMngSearchVO.getSearchKeyword());
...
function fncListSurveyMng(arg) {
	if (arg == "1")
		document.listForm.pageIndex.value = 1;
   	document.listForm.action = "<c:url value='/listSurveyMng.do' />";
   	document.listForm.submit();		
}

function fncAddSurveyMngView() {
    document.listForm.action = "<c:url value='/addSurveyMngView.do' />";
    document.listForm.submit();
}

function fncDetailSurveyMng(pollIdx) {
	document.listForm.pollIdx.value = pollIdx;
   	document.listForm.action = "<c:url value='/detailSurveyMng.do' />";
   	document.listForm.submit();
}
-->
</script>
...
<!--							<a href=""><img src="../../img/adminCenter/icon_month.gif" alt="등록일 달력"/></a>-->
						...
						<td rowspan="2" width="70px;"><div class="btnareaC"><span class="btn radiusG"><a href="javascript:fncListSurveyMng('1');">검색</a></span></div></td>
					</tr>
					<tr>
				...
				<div class="btnarea">
					...
					if (role.equals("GG101") || role.equals("GG102")) {
					%>
					<span class="btn radiusB"><a href="javascript:fncAddSurveyMngView();">등록</a></span>
					<%
					}
					%>
				</div>
				...
				<colgroup>
					...
					%>
						<tr>
							...
							<td class="tl"><a href="javascript:fncDetailSurveyMng('<%= pollMng.getPollIdx() %>');"><%= pollMng.getPollTitle() %></a></td>
							...
							<%
									...
									else if ("P".equals(pollMng.getPollStop())) {
							%>
							<td>진행</td>
							<%
									}
							...
							<td><%= EmpUtil.null2str(pollMng.getPollRestrict()) %></td>
						</tr>
						<%
								}
						%>
				...
				<div class="paginate">
					<anyframe:pagenavigator linkUrl="javascript:fncListSurveyMng(2);" 
							             	pages="<%=resultPage%>" 
							             	formName="listForm"
