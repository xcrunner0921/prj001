...
<%@ page import="seoul.research.admincenter.service.QuestMngSearchVO" %> 
...
<%
	QuestMngSearchVO questMngSearchVO = (QuestMngSearchVO)request.getAttribute("questMngSearchVO");
	String regDatestart = EmpUtil.null2str(questMngSearchVO.getSearchRegDatestart());
	String regDateend = EmpUtil.null2str(questMngSearchVO.getSearchRegDateend());
	String pollQuest = EmpUtil.null2str(questMngSearchVO.getSearchPollQuest());
	String pollAsk = EmpUtil.null2str(questMngSearchVO.getSearchPollAsk());
	String condition = EmpUtil.null2str(questMngSearchVO.getSearchCondition());
	String keyword = EmpUtil.null2str(questMngSearchVO.getSearchKeyword());
...
function fncListQuestMng(arg) {
	if (arg == "1")
		document.listForm.pageIndex.value = 1;
   	document.listForm.action = "<c:url value='/listQuestMng.do' />";
   	document.listForm.submit();		
}

function fncDetailQuestMng(pollIdx) {
	document.listForm.pollIdx.value = pollIdx;
   	document.listForm.action = "<c:url value='/detailQuestMng.do' />";
   	document.listForm.submit();
}

function fncAddQuestMngView(pollIdx) {
	document.listForm.pollIdx.value = pollIdx;
    document.listForm.action = "<c:url value='/addQuestMngView.do' />";
    document.listForm.submit();
}
-->
</script>
...
<!--							<a href=""><img src="../../img/adminCenter/icon_month.gif" alt="등록일 달력"/></a>-->
						...
						<th width="70px;"><label for="">문항작성</label></th>
						<td>
							<select name="searchPollQuest" id="">
								<option value="">전체</option>
								<option value="S" <%= ("S".equals(pollQuest) ? "selected" : "")%>>미작성</option>
								<option value="P" <%= ("P".equals(pollQuest) ? "selected" : "")%>>작성중</option>
								<option value="C" <%= ("C".equals(pollQuest) ? "selected" : "")%>>완료</option>
							</select>					
						</td>
						<td rowspan="2" width="70px;"><div class="btnareaC"><span class="btn radiusG"><a href="javascript:fncListQuestMng('1');">검색</a></span></div></td>
					</tr>
					<tr>
...
<!--					<span class="btn radiusB"><a href="#">등록</a></span>-->
				...
				<colgroup>
					...
					<tbody>
						...
						<tr>
							...
							<%
									if ("S".equals(pollMng.getPollQuest())) {
							%>
							<td class="tl"><%= pollMng.getPollTitle() %></td>
							<%
									}
									else {
							%>
							<td class="tl"><a href="javascript:fncDetailQuestMng('<%= pollMng.getPollIdx() %>');"><%= pollMng.getPollTitle() %></a></td>
							<%
									}
							%>
							...
							<%
									...
									else if ("P".equals(pollMng.getPollQuest())) {
							%>
							<td>작성중</td>
							<%
									}
									else if ("C".equals(pollMng.getPollQuest())) {
							%>
							<td>완료</td>
							<%
									}
					...
					if (role.equals("GG101") || role.equals("GG102")) {
					%>
								<span class="btn lightB"><a href="javascript:fncAddQuestMngView('<%= pollMng.getPollIdx() %>');">등록</a></span>
					<%
					}
					%>
							</td>
				...
				<div class="paginate">
					<anyframe:pagenavigator linkUrl="javascript:fncListQuestMng(2);" 
             								pages="<%=resultPage%>" 
             								formName="listForm"
